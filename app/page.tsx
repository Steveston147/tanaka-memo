...
type IconName = "spark" | "plus" | "search" | "pin" | "check" | "edit" | "trash" | "close" | "list" | "grid" | "inbox" | "clock" | "calendar" | "refresh" | "link" | "download" | "upload";
...
function matchesDateFilter(memo: Memo, filter: DateFilter) {
  ...
  if (filter === "overdue") return (memo.status || "active") === "active" && memoDate < today;
...
export default function Home() {
  ...
  const [dateFilter, setDateFilter] = useState<DateFilter>("all"); const [notice, setNotice] = useState(""); const [googleConnected, setGoogleConnected] = useState(false); const [googleBusy, setGoogleBusy] = useState(false);
  const titleRef = useRef<HTMLInputElement>(null); const importRef = useRef<HTMLInputElement>(null); const restoreRef = useRef<HTMLInputElement>(null); const googleConfigured = isGoogleCalendarConfigured();
  ...
  const syncMemo = async (memo: Memo) => { if (!memo.scheduleDate) return patchMemo(memo.id, { googleSyncStatus: "error", googleSyncError: "日付が必要です" }); if (!isGoogleCalendarConnected()) { setGoogleConnected(false); return patchMemo(memo.id, { googleSyncStatus: "error", googleSyncError: "Google Calendarに接続してください" }); } patchMemo(memo.id, { googleSyncStatus: "pending", googleSyncError: undefined }); try { const result = memo.googleEventId ? await updateGoogleCalendarEvent(memo.googleEventId, memo) : await createGoogleCalendarEvent(memo); patchMemo(memo.id, { googleEventId: result.id, googleEventUrl: result.htmlLink, googleSyncStatus: "synced", googleSyncError: undefined, googleLastSyncedAt: new Date().toISOString() }); flash("Google Calendarへ反映しました"); } catch (error) { patchMemo(memo.id, { googleSyncStatus: "error", googleSyncError: error instanceof Error ? error.message : "同期に失敗しました" }); if (!isGoogleCalendarConnected()) setGoogleConnected(false); } };
  const saveMemo = async (event: FormEvent) => { event.preventDefault(); const title = form.title.trim(); if (!title) return titleRef.current?.focus(); const memo: Memo = { ...form, title, description: form.description.trim(), id: createId(), createdAt: new Date().toISOString(), status: "active", pinned: false, googleSyncStatus: form.syncToGoogle ? "not-synced" : undefined }; setMemos(current => [memo, ...current]); setForm(initialForm); flash("メモを保存しました"); titleRef.current?.focus(); if (memo.syncToGoogle) await syncMemo(memo); };
  ...
  const removeMemo = async (memo: Memo) => { if (!confirm(`「${memo.title}」を削除しますか？`)) return; if (memo.googleEventId && confirm("Google Calendarの予定も削除しますか？")) { try { await deleteGoogleCalendarEvent(memo.googleEventId); } catch (error) { flash(error instanceof Error ? `Google側の削除に失敗：${error.message}` : "Google側の削除に失敗しました"); return; } } setMemos(current => current.filter(m => m.id !== memo.id)); flash("メモを削除しました"); };
  const exportBackup = () => { const blob = new Blob([JSON.stringify(memos, null, 2)], { type: "application/json" }); const url = URL.createObjectURL(blob); const anchor = document.createElement("a"); anchor.href = url; anchor.download = `work-board-${localDateKey(new Date())}.json`; anchor.click(); URL.revokeObjectURL(url); flash("バックアップを書き出しました"); };
  ...
  const importMemos = async (event: React.ChangeEvent<HTMLInputElement>) => { try { const imported = await readBackupFile(event); if (!imported) return; const currentIds = new Set(memos.map(m => m.id)); const added = imported.filter(m => !currentIds.has(m.id)).length; const updated = imported.length - added; if (!confirm(`${imported.length}件をImportします。\n新規 ${added}件・更新 ${updated}件\n現在のデータは削除されません。`)) return; setMemos(current => { const importedById = new Map(imported.map(m => [m.id, m])); const merged = current.map(m => importedById.get(m.id) ?? m); const existingIds = new Set(current.map(m => m.id)); return [...imported.filter(m => !existingIds.has(m.id)), ...merged]; }); flash(`Import完了：新規${added}件・更新${updated}件`); } catch { flash("正しいバックアップファイルではありません"); } };
  const restoreBackup = async (event: React.ChangeEvent<HTMLInputElement>) => { try { const imported = await readBackupFile(event); if (!imported) return; if (!confirm(`${imported.length}件を復元します。現在のデータはすべて置き換えられます。`)) return; setMemos(imported); flash(`${imported.length}件を復元しました`); } catch { flash("正しいバックアップファイルではありません"); } };

  const filtered = useMemo(() => memos.filter(m => { const q = query.trim().toLowerCase(); return (!q || `${m.title} ${m.description}`.toLowerCase().includes(q)) && (category === "all" || m.category === category) && (status === "all" || (m.status || "active") === status) && matchesDateFilter(m, dateFilter); }).sort((a,b) => { if (!!a.pinned !== !!b.pinned) return a.pinned ? -1 : 1; if (sort === "priority" && a.priority !== b.priority) return a.priority === "high" ? -1 : 1; if (sort === "schedule") return scheduleTimestamp(a) - scheduleTimestamp(b); return sort === "old" ? +new Date(a.createdAt) - +new Date(b.createdAt) : +new Date(b.createdAt) - +new Date(a.createdAt); }), [memos, query, category, status, sort, dateFilter]);
  ...
  return <main className="app-shell min-h-screen pb-20">
    <header className="glass sticky top-0 z-30 border-b border-slate-200/80"><div className="mx-auto flex min-h-16 max-w-[1480px] items-center justify-between gap-3 px-4 py-2 sm:px-7">
      ...
      <div className="flex items-center gap-2"><button onClick={exportBackup} className="rounded-xl bg-white p-2 text-slate-600 ring-1 ring-slate-200" title="バックアップを書き出す"><Icon name="download" className="h-4 w-4"/></button><button onClick={()=>importRef.current?.click()} className="rounded-xl bg-white px-3 py-2 text-xs font-bold text-slate-700 ring-1 ring-slate-200" title="現在のデータに追加・更新">Import</button><input ref={importRef} type="file" accept="application/json,.json" className="hidden" onChange={importMemos}/><button onClick={()=>restoreRef.current?.click()} className="rounded-xl bg-white px-3 py-2 text-xs font-bold text-rose-600 ring-1 ring-rose-200" title="現在のデータを置き換える">復元</button><input ref={restoreRef} type="file" accept="application/json,.json" className="hidden" onChange={restoreBackup}/>{googleConfigured ? googleConnected ? <button onClick={disconnectGoogle} className="rounded-xl bg-emerald-50 px-3 py-2 text-xs font-bold text-emerald-700 ring-1 ring-emerald-200">Google接続済み</button> : <button onClick={connectGoogle} disabled={googleBusy} className="rounded-xl bg-white px-3 py-2 text-xs font-bold text-slate-700 ring-1 ring-slate-200"><Icon name="calendar" className="mr-1 inline h-4 w-4"/>{googleBusy ? "接続中" : "Google Calendarに接続"}</button> : <span className="rounded-xl bg-amber-50 px-3 py-2 text-xs font-bold text-amber-700 ring-1 ring-amber-200">Google設定待ち</span>}</div>
    </div></header>
