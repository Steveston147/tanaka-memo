...
export default function Home() {
  ...
  const titleRef = useRef<HTMLInputElement>(null); const importRef = useRef<HTMLInputElement>(null); const googleConfigured = isGoogleCalendarConfigured();
  ...
  const importBackup = async (event: React.ChangeEvent<HTMLInputElement>) => { const file = event.target.files?.[0]; event.target.value = ""; if (!file) return; try { const parsed: unknown = JSON.parse(await file.text()); if (!Array.isArray(parsed)) throw new Error(); const imported = parsed.filter(validMemo); if (!imported.length && parsed.length) throw new Error(); if (!confirm(`${imported.length}件を読み込みます。現在のデータを置き換えますか？`)) return; setMemos(imported); flash(`${imported.length}件を復元しました`); } catch { flash("正しいバックアップファイルではありません"); } };
  ...
  return <main className="app-shell min-h-screen pb-20">
    <header className="glass sticky top-0 z-30 border-b border-slate-200/80"><div className="mx-auto flex min-h-16 max-w-[1480px] items-center justify-between gap-3 px-4 py-2 sm:px-7">
      ...
      <div className="flex items-center gap-2"><button onClick={exportBackup} className="rounded-xl bg-white p-2 text-slate-600 ring-1 ring-slate-200" title="バックアップ"><Icon name="download" className="h-4 w-4"/></button><button onClick={()=>importRef.current?.click()} className="rounded-xl bg-white p-2 text-slate-600 ring-1 ring-slate-200" title="復元"><Icon name="upload" className="h-4 w-4"/></button><input ref={importRef} type="file" accept="application/json,.json" className="hidden" onChange={importBackup}/>{googleConfigured ? googleConnected ? <button onClick={disconnectGoogle} className="rounded-xl bg-emerald-50 px-3 py-2 text-xs font-bold text-emerald-700 ring-1 ring-emerald-200">Google接続済み</button> : <button onClick={connectGoogle} disabled={googleBusy} className="rounded-xl bg-white px-3 py-2 text-xs font-bold text-slate-700 ring-1 ring-slate-200"><Icon name="calendar" className="mr-1 inline h-4 w-4"/>{googleBusy ? "接続中" : "Google Calendarに接続"}</button> : <span className="rounded-xl bg-amber-50 px-3 py-2 text-xs font-bold text-amber-700 ring-1 ring-amber-200">Google設定待ち</span>}</div>
    </div></header>
